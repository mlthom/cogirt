...
ylim <- c(
  rda$omega_mu[1, int_par_2] - 4 * rda$omega_sigma2[int_par_2, int_par_2],
  rda$omega_mu[1, int_par_2] + 4 * rda$omega_sigma2[int_par_2, int_par_2]
)
...
points(x = mcmh_est$omegaEAP[int_par_1], y = mcmh_est$omegaEAP[int_par_2],
       col = rgb(0,1,0,.5), pch = 20, cex = 4, xpd = T)
points(x = rmmh_est$omega1[int_par_1], y = rmmh_est$omega1[int_par_2],
       col = rgb(0,0,1,.5), pch = 20, cex = 4, xpd = T)
points(x = nr_est[int_par_1], y = nr_est[int_par_2],
       col = rgb(1,0,1,.5), pch = 20, cex = 4, xpd = T)

legend(x = xlim[1], y = ylim[2], yjust = -0.1, xpd = T, y.intersp = .8, legend = c(
  "True",
